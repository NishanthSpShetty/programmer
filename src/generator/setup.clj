...
(defn create-project-dir
  ...
  (let [dir (io/file dir-name)]
    ...
    (if (.mkdirs dir)
      dir
      (throw (java.io.IOException. "failed to create project directory, check if you have access or file already exist")))))

;; Miscellanies  file required for the projects
(defn write-misc-file
  [root-dir operator operator-str start end]
  (println "generating misc files...")
  (with-open [writer (io/writer (io/file root-dir "README.md"))]
    (let [content (str "## " root-dir "\n"
                       ...
                       "\n### " root-dir "\n"
                       "A program to perform " (name operator) " (" operator-str ") operation on given input numbers which is in range of " start " and " end
...
(defn create-project-files
  ...
  (.mkdirs (io/file root-dir "src/main"))
  
  (.mkdirs (io/file root-dir "test/main"))
  ;;create project.clj
...
(defn set-it-up
  ...
  (let [project-name (str (name operator) "_between_" start "_and_" end)
